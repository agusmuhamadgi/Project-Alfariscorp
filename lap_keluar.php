...
<a href="javascript:;" ><width="18" height="18" border="0" onClick="window.open('./excel/export_excel.php','scrollwindow','top=200,left=300,width=800,height=500');">PRINT </a>
			<table border="1" cellpadding="5" cellspacing="0" class="table">
				...
				$sql=" SELECT *	FROM mst_keluar	WHERE  pengirim LIKE '%{$search}%'";
             ...
             <div id="footer"> <!--koding untuk  mnjumlahkan total pembayaran per perusahaan-->
            	<?php
		$totalmasuk = mysql_query("SELECT *, sum(nominal) FROM mst_keluar WHERE pengirim LIKE '%{$search}%'") or die(mysql_error());
